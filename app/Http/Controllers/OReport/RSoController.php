...
class RSoController extends Controller
{
	...
	public function jasperSoReport(Request $request) 
	{
		...
		$query = DB::SELECT("
			SELECT NO_BUKTI,DATE_FORMAT(TGL,'%d/%m/%Y') AS TGL,KODEC,NAMAC,KD_BRG,NA_BRG,KG,HARGA,TOTAL,NOTES,KIRIM,SISA from so $filtertgl $filtergol $filterkodec  $filtersls $filterbrg;
		"); 
