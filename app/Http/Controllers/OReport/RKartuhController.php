...
class RKartuhController extends Controller
{
	...
	public function getHutKartu(Request $request)
    {
		...
		(
			...
			SELECT NO_BUKTI, DATE_FORMAT(belix.TGL,'%d/%m/%Y') AS TGL, KODES, NAMAS, PER$bulan AS TOTAL, 0 AS BAYAR 
			from belix where belix.YER='$tahun' $filterbelix union all

			SELECT NO_BUKTI, DATE_FORMAT(hut.TGL,'%d/%m/%Y') AS TGL, KODES, NAMAS, 0 AS TOTAL, BAYAR  
	...
	public function getHutSisa(Request $request)
    {
		...
		$query = DB::SELECT("
        	SELECT NO_BUKTI, DATE_FORMAT(TGL,'%d/%m/%Y') AS TGL, KODES, NAMAS, 
			(CASE 
	...
	public function jasperHutSisaReport(Request $request) 
	{
		...
		$query = DB::SELECT("
        	SELECT NO_BUKTI, DATE_FORMAT(TGL,'%d/%m/%Y') AS TGL , trim(KODES) as KODES, NAMAS, (PER$bulan-PERB$bulan) as SISA, FLAG
