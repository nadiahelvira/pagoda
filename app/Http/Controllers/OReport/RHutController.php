...
class RHutController extends Controller
{
	...
	public function jasperHutReport(Request $request) 
	{
		...
		$query = DB::SELECT("
			SELECT hut.NO_BUKTI,hut.BACNO,hut.BNAMA,hut.NO_PO,DATE_FORMAT(hut.TGL,'%d/%m/%Y') AS TGL,hut.KODES,hut.NAMAS,hutd.NO_FAKTUR,hutd.TOTAL,hutd.BAYAR,hutd.SISA,hut.GOL from hut,hutd WHERE hut.NO_BUKTI=hutd.NO_BUKTI $filtertgl $filtergol $filterkodes /*order by hut.KODES,hut.NO_BUKTI*/;
		");
