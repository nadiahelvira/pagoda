...
class RbankController extends Controller
{
    ...
    /*
		...
		*/
		...
		(
			SELECT '' AS NO_BUKTI, DATE_FORMAT('$tglDrD','%d/%m/%Y') AS TGL, BACNO AS BACNO, BNAMA AS BNAMA, '' AS ACNO, '' AS NACNO, 
			'SALDO AWAL' URAIAN, 
			...
			UNION ALL
			SELECT bank.NO_BUKTI, DATE_FORMAT(bank.TGL,'%d/%m/%Y') AS TGL, bank.BACNO, bank.BNAMA, bankd.ACNO, bankd.NACNO, bankd.URAIAN, 0 AWAL, bankd.DEBET, bankd.KREDIT 
	...
	public function jasperBankReport(Request $request) 
	{
		...
		(
		        ...
		        SELECT '' AS NO_BUKTI, DATE_FORMAT('$tglDrD','%d/%m/%Y') AS TGL, '$xbacno1' as BACNO, '' AS BNAMA, '' AS ACNO, '' AS NACNO, 'Saldo Awal' AS URAIAN, 
		        '$xawal1' AS AWAL, 0 AS DEBET, 0 AS KREDIT, 1 AS URUTAN  
		        UNION ALL  
			SELECT bank.NO_BUKTI, DATE_FORMAT(bank.TGL,'%d/%m/%Y') AS TGL, bank.BACNO, bank.BNAMA, bankd.ACNO, bankd.NACNO, bankd.URAIAN, 0 AWAL, bankd.DEBET, bankd.KREDIT, 2 AS URUTAN
			from bank, bankd where bank.NO_BUKTI=bankd.NO_BUKTI and bank.TGL BETWEEN '$tglDrD' and '$tglSmpD' and bank.BACNO='$acno' and bank.PER='$periode'  
